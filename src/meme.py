...
This script allows for the generation of memes by adding
quotes to images. The script can either take a random image
and quote or use a user-supplied image and quote. The
...
Functions:
    - generate_meme: Generates a meme using a
                     random or user-provided image and quote.
    - parse_args: Parses command-line arguments
                  to get image path, quote body, and author.
"""
...
def generate_meme(path=None, body=None, author=None):
    ...
    This function generates a meme by adding a quote to an image.
    The image can be randomly chosen from a predefined set of
    dog images or provided by the user through the `path` argument.
    Similarly, the quote can either be randomly selected from
    a set of quote files or provided through the `body` and `author` arguments.

    Parameters:
        path (str, optional): The path to the image file.
                              If None, a random image is selected.
        body (str, optional): The body of the quote to overlay on the image.
                              If None, a random quote is used.
        author (str, optional): The author of the quote.
                                Required if `body` is provided.
...
def parse_args():
    ...
    passed to the script. It allows the user to specify a custom image path,
    quote body, and author to generate a meme.
    If no arguments provided, default behavior is random images and quotes.
